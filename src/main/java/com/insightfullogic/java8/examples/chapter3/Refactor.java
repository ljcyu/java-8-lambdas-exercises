...
public class Refactor {
    ...
    public static class Step0 implements LongTrackFinder {
        ...
        public Set<String> findLongTracksBySelf(List<Album> albums) {
            Set<String> res1= albums.stream()
                    .flatMap(album ->
                            album.getTrackList().stream()
                                    .filter(track -> track.getLength() > 60)
                                    .map(Track::getName))
                    .collect(toSet());
            Set<String> res2=albums.stream().flatMap(album -> album.getTrackList().stream())
                    .filter(track->track.getLength()>60)
                    .map(Track::getName)
                    .collect(toSet());
            return res2;
...
// END findLongTracks_4
    ...
    public static void main(String[] args) {
        Refactor.Step0 refactor = new Refactor.Step0();
        System.out.println(refactor.findLongTracks(SampleData.getThreeAlbums()));
        System.out.println(refactor.findLongTracksBySelf(SampleData.getThreeAlbums()));
    }
}
